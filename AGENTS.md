...
- Synology deployment safety checkpoint:
  ...
  - Current Synology assistant chat path uses the Mac-hosted CLI bridge over a private SSH reverse tunnel:
    ...
    - Latest verified API smoke test on Synology returned HTTP 200 from `POST /api/assistant/chat` using `provider=codex-cli` after the timeout fallback patch. It completed in 37 seconds end-to-end, provider time was 30.6 seconds, returned status `success` with 4 actions, and wrote `/data/assistant-audits/assistant-chat/2026-07-22T00-55-59-160Z-assistant-chat-fcd73497-9281-44cc-b9f5-b1c7348c2daa.json`.
  ...
  - Runtime issue fixed after browser 504s on `codex-cli`: assistant chat now caps local CLI provider wait time at 35 seconds, returns a deterministic Immich coverage-plan fallback instead of letting the reverse proxy/browser 504, and no longer makes a second LLM call after auto-running read-only tool actions. Auto-tool results are summarized directly into the current response and remain available through JSON logs.
  - The running Synology container was hot-patched at `/usr/src/app/server/dist/services/assistant.service.js`; the previous runtime file was backed up inside the container as `/tmp/assistant.service.js.pre-timeout-fallback.20260721-184622`.
  - Runtime issue fixed after simple `hello` still waited on Codex: assistant chat now has a pre-context fast path for simple status/greeting messages. It returns before loading library context or calling the provider. Verified authenticated Synology smoke: `hello` returned HTTP 200 in 0 seconds, diagnostic `/data/assistant-audits/assistant-chat/2026-07-22T01-01-54-748Z-assistant-chat-c59b9b08-cf33-436c-8e55-35472ebe7b86.json` recorded `fastPath=simple_status`, `durationMs=1`, and `providerAttempts=[]`.
  - The running Synology container was hot-patched again at `/usr/src/app/server/dist/services/assistant.service.js`; the previous runtime file was backed up inside the container as `/tmp/assistant.service.js.pre-fast-hello.20260721-190115`.
