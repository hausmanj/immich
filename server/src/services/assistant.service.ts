  ...
  private async getLibraryContext(auth: AuthDto, dto?: AssistantChatRequestDto) {
    ...
    return {
      ...
      deterministicAudits: {
        ...
        actionGuidance:
          'For reversible review albums, use organizationCoveragePlan to account for the whole library. Prefer eventCohorts for GPS/place-anchored multi-day trips, then cover the older no-GPS majority with sourcePathCohorts/dateCohorts and camera audits. Source path cohorts are exact source directories; if a folder is very large or has high generatedLikeCount/smallDimensionCount, decompose it into child folders/noise audits instead of treating the parent as one album. Never treat No visible location as a reason to leave assets unaddressed. Event cohort assetCount is the materialized review size and includes compatible no-location/date/source-folder support assets; locationAssetCount is only the GPS/place-labeled anchor count. Use cohortType and cohortKey from deterministic cohorts instead of enumerating large asset ID lists. For deeper evidence, propose a read-only tool action with toolType/toolInput so Immich can run content_hash_audit, sidecar_pair_audit, metadata_search, or mobile_original_compare.',
      },
  ...
  private toOrganizationCoveragePlan(
    ...
    return {
      ...
      selectionRules: [
        ...
        'If a source directory is very large or has high generatedLikeCount/smallDimensionCount, break it down with read-only audits before proposing final review albums.',
