...
export class AssistantService extends BaseService {
    ...
    return {
      ...
      capabilities: [
        ...
        },
        {
          ...
          undoSupported: true,
          journalRequired: true,
          notes: 'Trashes the group loser assets (soft-delete, reversible). Undo restores them from trash. Keeper assets and files on disk are never deleted; hard delete is never performed.',
        },
      ],
    };
  }
  ...
  private toFastAssistantChatResponse(
    ...
    const isSimpleStatus =
      ...
      (/^(hi|hello|hey)\b/.test(normalized) &&
        /\b(status|working|ready|there|online|ok|okay|test|ping)\b/.test(normalized) &&
  ...
  private async getAssistantMutationBeforeState(
    ...
    switch (dto.actionType) {
      ...
      case 'duplicate_resolution': {
        const trashAssetIds = this.requireDuplicateResolutionTrashAssetIds(dto);
        return {
          duplicateResolution: dto.duplicateResolution ?? null,
          trashAssetIds,
          assets: await this.getAssistantAssetSnapshots(auth, trashAssetIds),
        };
      }
    }
  }
  ...
  private async applyAssistantMutation(auth: AuthDto, dto: AssistantMutationRequestDto): Promise<{
    ...
    switch (dto.actionType) {
      ...
      case 'duplicate_resolution': {
        const trashAssetIds = this.requireDuplicateResolutionTrashAssetIds(dto);
        await this.requireAccess({ auth, permission: Permission.AssetDelete, ids: trashAssetIds });
        await BaseService.create(AssetService, this).deleteAll(auth, { ids: trashAssetIds, force: false });
        return {
          after: { trashedAssetIds: trashAssetIds, status: 'trashed' },
          undo: {
            strategy: 'restore_trashed_assets',
            available: true,
            albumId: null,
            albumName: null,
            trashAssetIds,
          },
          message: `Trashed ${trashAssetIds.length} duplicate loser assets (soft-delete, reversible via undo). Keeper assets untouched.`,
        };
      }

      case 'folder_move': {
        throw new BadRequestException(
          'folder_move apply stays blocked in Immich: physical file reorganization runs through tools/photo-file-organizer.mjs (journaled plan/apply/undo) so external-library paths are not desynced.',
        );
      }
    }
  }
  ...
  private async applyAssistantUndo(
    ...
    switch (journal.undo.strategy) {
      ...
      case 'restore_trashed_assets': {
        const trashAssetIds = journal.undo.trashAssetIds ?? [];
        if (trashAssetIds.length === 0) {
          throw new BadRequestException('Assistant duplicate-resolution undo is missing trashed asset IDs');
        }
        await BaseService.create(TrashService, this).restoreAssets(auth, { ids: trashAssetIds });
        return {
          targetId: trashAssetIds[0],
          targetName: null,
          message: `Restored ${trashAssetIds.length} trashed duplicate assets from trash.`,
          restoredAssetCount: trashAssetIds.length,
        };
      }
  ...
  private requireDuplicateResolutionTrashAssetIds(dto: AssistantMutationRequestDto): string[] {
    const groups = dto.duplicateResolution?.groups ?? [];
    const trashAssetIds = this.toUniqueStrings(groups.flatMap((group) => group.trashAssetIds ?? []));
    if (trashAssetIds.length === 0) {
      throw new BadRequestException('duplicate_resolution requires at least one trashAssetId across the provided groups');
    }
    const keepAssetIds = new Set(this.toUniqueStrings(groups.flatMap((group) => group.keepAssetIds ?? [])));
    const overlap = trashAssetIds.filter((id) => keepAssetIds.has(id));
    if (overlap.length > 0) {
      throw new BadRequestException(`duplicate_resolution cannot trash and keep the same asset: ${overlap.join(', ')}`);
    }
    return trashAssetIds;
  }
