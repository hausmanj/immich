...
export class AssistantService extends BaseService {
    ...
    const remainingSourcePathReviewCohorts: AssistantOrganizationCoverageItem[] = sourcePathCohorts
      ...
      .map((cohort) => {
        ...
        const reviewStrategy =
          cohort.assetCount >= 1000 || generatedRatio >= 0.25 || smallDimensionCount >= 250
            ? ('decompose_first' as const)
            : ('review_album' as const);
        ...
        return {
          ...
          rationale:
            reviewStrategy === 'decompose_first'
              ? `Large or noise-heavy source directory: ${generatedLikeCount} generated-like assets and ${smallDimensionCount} small-dimension assets. Decompose with read-only audits before proposing albums.`
  ...
  private toCoverageDecompositionToolType(
    ...
    if (generatedRatio >= 0.5 || /movies?|videos?|copies|duplicate/i.test(item.cohortKey)) {
