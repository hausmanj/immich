...
- **Two consoles, one bridge:**
  - Immich-embedded: **2026-08-31 update** — the hot-patched standalone `agent-console.html` /
    `patch-assistant-controller.mjs` path described below has been superseded by a real Svelte route,
    `/assistant` (`web/src/routes/(user)/assistant/`) backed by `assistant.controller.ts`, both on `main`.
    That standalone source was dropped when merging this branch's dedup/RAW-handling work forward — the
    bridge itself (`assistant-cli-bridge.mjs`, SSE streaming, prompt archiving) is unaffected and current.
    ~~Previously: `<immich-url>/api/assistant/agent-console`, served by hot-patching the running
